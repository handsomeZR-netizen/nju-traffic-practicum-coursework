...
def get_agent(name):
    from agent import CoLight, EcoLight, FixedTime, FRAP, MaxPressure, MPLight, SOTL, TinyLight, RandomPath, PiRule
    name_2_class = {
        'CoLight': CoLight,
        'EcoLight': EcoLight,
        'FixedTime': FixedTime,
...
def set_logger(config):
    if config['save_result']:
        ...
        if config['flow_idx'] == 0:
            with open('{}/config.json'.format(config['log_path']), 'w') as fout:  # 把 config保存在log_path中
...
def list_with_unique_element(original_list):  # list去重
    new_list = []
