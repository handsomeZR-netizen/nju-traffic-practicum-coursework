...
from agent_diy.feature.definition import ActData, FeatureProcess, ObsData, get_webster_lane_group, on_enter_lane
...
class Agent(BaseAgent):
    ...
    def __predict_detail(self, list_obs_data, exploit_flag=False):
        ...
        for i in range(len(legal_action)):
            ...
            if exploit_flag and getattr(list_obs_data[i], "rule_action", None) is not None:
                action = list(list_obs_data[i].rule_action)
    ...
    def _collect_phase_metrics(self, vehicles):
        ...
        for vehicle in vehicles:
            ...
            metric["delay_sum"] += delay_proxy
            metric["approach_pressure"] += approach_gain
    ...
    def _build_phase_scores(self, phase_metrics):
        ...
        for phase_id, metric in phase_metrics.items():
            ...
            score = (
                metric["close_count"] * Config.CLOSE_PRESSURE_COEF
                + metric["very_close_count"] * Config.VERY_CLOSE_PRESSURE_COEF
    ...
    def _build_signal_plan(self, phase_metrics, phase_scores, scene_waiting_vehicle_count):
        ranked_phases = sorted(
            ...
            key=lambda phase_id: (
                ...
                phase_metrics[phase_id]["close_count"],
        ...
        if not active_phases:
            ...
            if self.last_phase is None:
                ...
                self.last_score_gap = phase_scores[best_phase]
            else:
                ...
                starved_phase = next(
                    (
                        ...
                        and phase_scores[phase_id] >= current_score * Config.STARVATION_OVERRIDE_RATIO
                    ),
                    None,
                )
    ...
    def _select_duration_action(self, next_phase, ranked_phases, phase_metrics, phase_scores, scene_waiting_vehicle_count):
        ...
        if selected_metric["vehicle_count"] == 0:
            ...
            selected_metric["very_close_count"] >= 4
            or selected_metric["close_count"] >= 8
            or selected_metric["queue_count"] >= 8
            or selected_metric["avg_wait"] >= 12.0
            or scene_waiting_vehicle_count >= 180
        ):
            duration_action = 3
        elif (
            selected_metric["close_count"] >= 5
            or selected_metric["queue_count"] >= 5
            or selected_metric["avg_wait"] >= 7.0
            or score_gap >= 4.0
        ):
            duration_action = 2
        elif (
            selected_metric["close_count"] >= 2
            or selected_metric["queue_count"] >= 2
        ...
        if (
            ...
            and score_gap >= 6.0
            and selected_metric["close_count"] >= 4
        ):
    ...
    def _build_observation(self, phase_metrics, phase_scores, scene_waiting_vehicle_count):
        ...
        for phase_id in range(Config.DIM_OF_ACTION_PHASE_1):
            ...
            observation.extend(
                [
                    min(metric["close_count"] / 10.0, 1.0),
                    min(metric["queue_count"] / 10.0, 1.0),
        ...
        for phase_id in range(Config.DIM_OF_ACTION_PHASE_1):
            observation.append(min(phase_scores[phase_id] / 35.0, 1.0))
        ...
        total_close = sum(metric["close_count"] for metric in phase_metrics.values())
        observation.extend(
            [
                min(total_close / 24.0, 1.0),
                min(scene_waiting_vehicle_count / 300.0, 1.0),
                min(self.same_phase_count / 4.0, 1.0),
                float(np.clip(self.last_score_gap / 10.0, -1.0, 1.0)),
            ]
        )
        return observation
    ...
    def _build_logit_bias(self, phase_scores, duration_action):
        ...
        duration_bias = np.full(Config.DIM_OF_ACTION_DURATION_1, -0.1, dtype=np.float32)
