...
# To run the train_test, you must modify the algorithm name here. It must be one of ppo, diy, pielight.
# Simply modify the value of the algorithm_name variable.
# 运行train_test前必须修改这里的算法名字, 必须是ppo、diy、pielight里的一个, 修改algorithm_name的值即可
algorithm_name_list = ["ppo", "diy", "pielight"]
algorithm_name = "ppo"
...
def validate_required_paths():
    required_paths = [
        project_path("conf", "kaiwudrl", "learner.toml"),
        project_path("conf", "kaiwudrl", "configure.toml"),
        project_path("conf", "kaiwudrl", "aisrv.toml"),
        project_path("tools", "change_sample_server.sh"),
        project_path("tools", "modelpool_start.sh"),
        project_path("tools", "stop.sh"),
    ]
    missing_paths = [path for path in required_paths if not os.path.exists(path)]
    if missing_paths:
        missing_text = "\n".join(missing_paths)
        raise FileNotFoundError(
            "The project package uploaded to the cloud is incomplete. "
            "Please upload the missing files or directories before running train_test.py:\n"
            f"{missing_text}"
        )
...
def train():
    ...
    # Make every relative path in the training scripts resolve from the project root.
    # 统一将训练脚本中的相对路径解析到项目根目录。
    ...
    if algorithm_name not in algorithm_name_list:
        ...
        python_exec_shell(f"sh {project_path('tools', 'stop.sh')} all")
    ...
    if sample_tool_type is None:
        ...
        result_code, result_str = python_exec_shell(
            f"sh {project_path('tools', 'change_sample_server.sh')} {sample_tool_type}"
        )
        if result_code != 0:
            raise ValueError(f"Execution error! Please check the error detail: {result_str}")

    CONFIG.set_configure_file(project_path("conf", "kaiwudrl", "learner.toml"))
    ...
    python_exec_shell(f"sh {project_path('tools', 'modelpool_start.sh')} learner")
