...
class EcoLight(BaseDRL):
    ...
    def pick_action(self, n_obs, on_training):
        obs = n_obs[self.idx]  # obs是 list of tensor
...
class _Network(torch.nn.Module):
    ...
    def forward(self, obs):
        # obs[0]: _inter_2_current_phase, current phase, B * num_phase;
        # obs[1]: _inlane_2_num_vehicle, number of cars in each lane, B * num_lane
...
# 这个模型一局的第一步决策有问题，
# 他以为当前相位是0, 但他实际做的动作与上一局最后有关。一直到它换了一个动作才正常
# 0一开始也没多少车肯定会被换掉的
