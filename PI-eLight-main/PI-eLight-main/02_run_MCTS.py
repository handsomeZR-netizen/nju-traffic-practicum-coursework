...
from utilities.utils import set_seed, get_config, set_logger, release_logger, make_dir
from utilities.snippets import run_an_episode
# from agent import MCTS_synthesizer, PiPolicy
from agent.pi_light.adv_MCTS import MCTS_synthesizer
from agent.pi_light.adv_base import PiPolicy
import agent.pi_light.adv_program as adv_program
...
parser.add_argument('--search_num', type=bool, default=64)
parser.add_argument('--p_mode', type=str, default='two', help='[one, two, share]')
...
p_mode = args.p_mode
adv_program.ProgramMode = p_mode
...
def run_an_experiment(inter_name, flow_idx, seed):
    ...
    for idx in range(env.n):
        agent = PiPolicy(config, env, idx, p_mode)
        env.n_agent.append(agent)
    synthesizer = MCTS_synthesizer(env, config)
    synthesizer.begin_search(search_num)
    synthesizer.distribute(True)
    ...
    if not eval_generalization:
        ...
        for idx in range(target_env.n):
            agent = PiPolicy(config, target_env, idx, p_mode)
            target_env.n_agent.append(agent)
        synthesizer.n_agent = target_env.n_agent
        synthesizer.distribute()
...
if __name__ == '__main__':
    ...
    if not eval_generalization:
        print(f'dataset:{data_name}, method:{cur_agent}, mode:{p_mode}')
    else:
        print(f'source:{data_name}->target:{transfer_target}, method:{cur_agent}, mode:{p_mode}')
