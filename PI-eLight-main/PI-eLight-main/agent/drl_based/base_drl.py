...
class BaseDRL(BaseAgent):
    ...
    def predict_q(self, obss: list):  # 不是obs的list
        # Mplight print(obs[0].shape, obs[1].shape) torch.Size([1, 6]) torch.Size([1, 16]) 第一个one-hot,第二个float
        q_values = []
        self.network_local.eval()
        for obs in obss:
            with torch.no_grad():
                b_q_value = self.network_local(obs)
                q_value = b_q_value.view(-1).cpu().numpy()
