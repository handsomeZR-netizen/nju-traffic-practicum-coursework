...
class MaxPressure(BaseAgent):
    ...
    def pick_action(self, n_obs, on_training):
        obs = n_obs[self.idx]
        if self.inter.current_phase_time < self.t_min:  # current_phase_time的选项0\20\40\17\37
    ...
    def _get_pressure_for_phase(self, obs, phase_id):
        # obs是一个交叉口的
        # 该算法的特征是 lane_2_num_vehicle
        obs = obs[0]  # 取一类特征
        pressure = 0
        n_available_lane_link = self.inter.n_phase[phase_id].n_available_lanelink_id  # 每个phase对应的 n_available_lanelink_id
