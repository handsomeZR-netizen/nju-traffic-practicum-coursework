...
def run_a_step(env, policy, n_obs):  # n_obs中一个agent的观察也是list, 里面的元素要么是 N维的array, 要么是 (1, N)的tensor
    rollout = []
    n_action = []
    for agent in policy:
...
def _sample(obss, acts, qs, max_pts, is_reweight):  # Q用来改变采样方式的
    # Step 1: Compute probabilities
    # obss, qs 应该是二维矩阵
    ps = np.max(qs, axis=1) - np.min(qs, axis=1)  # 每个obs的最大Q和最小Q的差值
    ps = ps / np.sum(ps)
...
def to_numpy(obs: list):
    ...
    return np.concatenate(obs, axis=0)  # (26)
...
def identify_best_policy(env, config, policies):
    ...
    return policies[0][0], info  # todo 返回info


# see https://arxiv.org/abs/1805.08328
def train_dagger(env, config, teacher, student):
    ...
    qs.extend(teacher[0].predict_q([obs for obs, _ in trace]))  # teacher预测每个obs的q
    # qs中的每个元素是动作维度的向量
