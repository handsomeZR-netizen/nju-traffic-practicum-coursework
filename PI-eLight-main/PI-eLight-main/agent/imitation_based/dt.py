...
def save_dt_policy_viz(dt_policy, dirname, fname):
    ...
    # 使用 Graphviz 工具将 DOT 文件转换为图像
    with open(file) as dot_file:
        dot_data = dot_file.read()
    graph = graphviz.Source(dot_data)
    graph.render(file)  # 这将生成一个名为 "tree.pdf" 的图像文件
...
class DTPolicy:
    ...
    def train(self, obss, acts, train_frac):  # train_frac是训练集的比例
    ...
    def pick_action(self, obss, on_training=0):
        return self.tree.predict(obss)  # obss一定得是二维的
